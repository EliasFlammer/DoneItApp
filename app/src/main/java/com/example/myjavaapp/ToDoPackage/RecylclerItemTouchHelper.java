...
public class RecylclerItemTouchHelper extends ItemTouchHelper.SimpleCallback {
    ...
    //constructor
    public RecylclerItemTouchHelper(ToDoAdapter adapter) {
        super(0, ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT);
        this.adapter = adapter;
    }
    //this makes that i cant "drag and drop" tasks
    @Override
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive){
        ...
        if(dX>0){
            ...
            background = new ColorDrawable(ContextCompat.getColor(adapter.getContext(), R.color.colorPrimary)); //maybe wrong color
        } else {
